...
class RekapKuesionerAtasanSheet implements FromArray, WithTitle, WithEvents, ShouldAutoSize, WithColumnFormatting
{
    protected $project;
    protected $remark = 'Atasan';
    protected $groupedLevels;
    protected $headerRowCount = 3;
    ...
    {
        return 'Rekap Kuesioner Atasan';
    }

    public function array(): array
    {
        $exportData = [];
        $headerRow1 = ['No.', 'Nama Peserta', 'NIP', 'Nama Atlas'];
        $headerRow2 = ['', '', '', ''];
        $headerRow3 = ['', '', '', ''];
        ...
        $responden = DB::table('project_responden')
            ->where('project_id', $this->project->id)
            ->where('status_pengisian_kuesioner_atasan', 'Sudah')
            ->get();

        foreach ($responden as $index => $respondenItem) {
            $rowData = [$index + 1, $respondenItem->nama, $respondenItem->nip, $respondenItem->nama_atasan];
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $currentColIndex = 4; // Start from E for Atasan
